...
function FormGroup({
    input_type, on_change,
    elRef, input_name, place_holder,
    input_id, input_key, error,
    is_readonly, input_value, label_value = "", div_classname, style, label_style = {}, clickable_label = {}, input_class_name = "", has_addon = {},
}: any) {
    ...
    return (
        <>
            <div className={div_classname ?? "form-group"}>
                ...
                <>
                    {inputItem}
                    {(!isNullOrUndefined(error) && error?.hasError) || (input_type === "number" && input_value < 0) ? <span className="text-danger fs-7">{error?.message}</span> : []}
                </>
            </div>
        </>
    );
